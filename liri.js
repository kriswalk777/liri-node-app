...
function movieInfo(movie) {
	...
	var queryUrl = "https://www.omdbapi.com/?i=tt3896198&apikey=b4743335=" + movie; // using search because sometimes just getting a movie by name gives strange results
		...
		if (body && JSON.parse(body).Search && JSON.parse(body).Search.length > 0) {
			for (var i = 0; i < JSON.parse(body).Search.length; i++) {
				...
				if (result.Title.toLowerCase() === movie.toLowerCase()) {
		            var cont = false;
					var innerQueryURL = "https://www.omdbapi.com/?i=" + result.imdbID + "&apikey=b4743335";
